...
# ECS Task Role - for app-level AWS access (S3, Rekognition)
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:rekognition:${var.aws_region}:${data.aws_caller_identity.current.account_id}:collection/picmeup-*"
      }
    ]
  })
}

# GitHub Actions OIDC - for CI/CD without long-lived keys
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "token.actions.githubusercontent.com:sub" = "repo:murillocg/picmeup:*"
          }
        }
      }
    ]
