...
2. **We cannot email invitations today.** SES in the production account (`265213224417`) is in the sandbox, so any unverified recipient is rejected. Invitations are therefore delivered out of band — the admin tells the person "go to www.elitesportphotos.com and sign in with Google". If a password-based invite is added later, Cognito's own email service sends it (separate from our SES, ~50/day), which sidesteps the sandbox entirely.

   Note that the sandbox here has **never been appealed**: `ReviewDetails` is `None` in the production account. A denied case (177980975400037) exists in a *different* AWS account and does not apply. Even if production access is later granted, the out-of-band invite is worth keeping — no expiring links, no tokens to leak or re-send.
...
- **Customer-facing email.** Buyers receive no receipt or download link, and cannot until SES production access is granted. In the production account (`265213224417`) this has never been requested — `ReviewDetails` is `None` and `EnforcementStatus` is `HEALTHY` — so it is a first application, not an appeal. Before applying, give the account a configuration set with a bounce/complaint event destination (there is none) and stop `EmailService` swallowing send failures, since "how do you handle bounces and complaints" is asked directly. None of this blocks the work in this plan.
